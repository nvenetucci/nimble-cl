...
// Parts
app.get("/api/parts/:make", (req, res) => {
  const fetchResults = async () => {
    try {
      const response = await fetch(
        `https://portland.craigslist.org/d/auto-parts-by-owner/search/pts?nearbyArea=2&nearbyArea=232&nearbyArea=233&nearbyArea=246&nearbyArea=350&nearbyArea=94&query=${req.params.make}&searchNearby=2&sort=date`
      );
      const html = await response.text();
      const $ = cheerio.load(html);
      const results = [];
